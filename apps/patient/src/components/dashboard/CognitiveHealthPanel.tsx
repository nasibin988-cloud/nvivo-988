...
import { Brain, Heart, Activity, FileText, Calendar, TrendingUp, TrendingDown, Minus, ChevronRight } from 'lucide-react';
...
// Helper to calculate trend direction from data array
...
// Date formatter - mm/dd/yy format
...
function getDassLabel(score: number, thresholds: { normal: number; mild: number; moderate: number }): string {
...
function getDassColor(score: number, thresholds: { normal: number; mild: number; moderate: number }): string {
    ...
    <div className="relative group">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-surface via-surface to-surface-2 backdrop-blur-2xl rounded-theme-xl border border-border p-5 shadow-card">
        ...
        <div className="relative z-10 space-y-5">
          ...
          {/* PILLAR 1: Brain Health & Cognition */}
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-1.5 bg-surface-3 rounded-full overflow-hidden border border-border/30">
                      <div
                        ...
                        style={{
                          width: `${Math.min(((mentalHealth?.dass21?.depression ?? 0) / 42) * 100, 100)}%`,
                          backgroundColor: getDassColor(mentalHealth?.dass21?.depression ?? 0, { normal: 9, mild: 13, moderate: 20 })
                        }}
                      />
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-1.5 bg-surface-3 rounded-full overflow-hidden border border-border/30">
                      <div
                        className="h-full rounded-full transition-all duration-700"
                        style={{
                          width: `${Math.min(((mentalHealth?.dass21?.anxiety ?? 0) / 42) * 100, 100)}%`,
                          backgroundColor: getDassColor(mentalHealth?.dass21?.anxiety ?? 0, { normal: 7, mild: 9, moderate: 14 })
                        }}
                      />
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-1.5 bg-surface-3 rounded-full overflow-hidden border border-border/30">
                      <div
                        className="h-full rounded-full transition-all duration-700"
                        style={{
                          width: `${Math.min(((mentalHealth?.dass21?.stress ?? 0) / 42) * 100, 100)}%`,
                          backgroundColor: getDassColor(mentalHealth?.dass21?.stress ?? 0, { normal: 14, mild: 18, moderate: 25 })
                        }}
                      />
                    </div>

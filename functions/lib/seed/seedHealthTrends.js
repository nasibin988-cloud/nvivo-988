...
exports.seedHealthTrends = seedHealthTrends;
exports.clearHealthTrends = clearHealthTrends;
 ...
 */
function getPatternValue(pattern, progress, // 0 to 1 (start to end)
 ...
 */
function generateMetricData(config, daysToSeed) {
    const dataPoints = [];
    for (let daysAgo = daysToSeed; daysAgo >= 0; daysAgo--) {
        ...
        if (config.dailyFrequency === 1 || daysAgo % config.dailyFrequency === 0) {
            ...
            dataPoints.push({
                date: (0, dateUtils_1.formatDateYYYYMMDD)(date),
                value: roundedValue,
                timestamp: date,
            });
        }
 ...
 * Seed health trends data
 */
async function seedHealthTrends(options) {
    var _a, _b;
    ...
    await healthTrendsRef.doc('_config').set({
        ...
        lastSeeded: firestore_1.FieldValue.serverTimestamp(),
    ...
    for (const config of METRIC_CONFIGS) {
        ...
        if (points.length > 0) {
            ...
            const weekAgo = (_b = (_a = points[Math.max(0, points.length - 8)]) === null || _a === void 0 ? void 0 : _a.value) !== null && _b !== void 0 ? _b : latest;
 ...
 */
async function clearHealthTrends(patientId) {
    const db = (0, firestore_1.getFirestore)();
    const healthTrendsRef = db.collection('patients').doc(patientId).collection('healthTrends');
...
//# sourceMappingURL=seedHealthTrends.js.map
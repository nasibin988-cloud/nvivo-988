 ...
 */
function calculateBeneficialScore(evaluations) {
    const beneficialEvals = evaluations.filter((e) => KEY_BENEFICIAL_NUTRIENTS.includes(e.nutrientId) && e.percentOfTarget !== null);
    if (beneficialEvals.length === 0)
        return 0;
    // Calculate average percentage of targets met (capped at 100%)
    const totalPercent = beneficialEvals.reduce((sum, e) => {
        var _a;
        const percent = Math.min((_a = e.percentOfTarget) !== null && _a !== void 0 ? _a : 0, 100);
        return sum + percent;
    }, 0);
 ...
 */
function calculateLimitScore(evaluations) {
    var _a, _b;
    const limitEvals = evaluations.filter((e) => KEY_LIMIT_NUTRIENTS.includes(e.nutrientId) &&
        (e.percentOfLimit !== null || e.percentOfTarget !== null));
    if (limitEvals.length === 0)
        return 40; // No limit data = full points
    let totalScore = 0;
    ...
    for (const nutrientId of KEY_LIMIT_NUTRIENTS) {
        ...
        const percentOfLimit = (_b = (_a = evalResult.percentOfLimit) !== null && _a !== void 0 ? _a : evalResult.percentOfTarget) !== null && _b !== void 0 ? _b : 0;
 ...
 */
function calculateBalanceScore(evaluations) {
    var _a, _b, _c;
    let score = 0;
    ...
    if (protein && carbs && fat) {
        const proteinPct = (_a = protein.percentOfTarget) !== null && _a !== void 0 ? _a : 0;
        const carbsPct = (_b = carbs.percentOfTarget) !== null && _b !== void 0 ? _b : 0;
        const fatPct = (_c = fat.percentOfTarget) !== null && _c !== void 0 ? _c : 0;
        // Check if all macros are in reasonable range (50-150% of target)
        const macrosBalanced = proteinPct >= 50 &&
            proteinPct <= 150 &&
            carbsPct >= 50 &&
            carbsPct <= 150 &&
            fatPct >= 50 &&
        ...
        if (macrosBalanced) {
            ...
            if (avgMacroPct >= 67)
                score += 5;
            else if (avgMacroPct >= 33)
                score += 2;
        }
    }
    // Variety (10 points)
    // Count nutrients with at least 25% of target
    const nutrientsWithIntake = evaluations.filter((e) => { var _a; return e.classification === 'beneficial' && ((_a = e.percentOfTarget) !== null && _a !== void 0 ? _a : 0) >= 25; }).length;

...
exports.batchSearchOFF = batchSearchOFF;
 ...
 */
async function batchSearchOFF(queries) {
 ...
 */
function mapOFFToNutrition(nutriments) {
    const nutrition = {};
    const reliableNutrients = [];
    ...
    for (const [key, value] of Object.entries(nutrition)) {
        if (typeof value === 'number') {
            ...
            if (['calories', 'sodium', 'potassium', 'calcium', 'cholesterol', 'phosphorus', 'magnesium'].includes(k)) {
                scaled[k] = Math.round(value * scale);
            }
            else {
                scaled[k] = round1(value * scale);
            }
        }
    }
    return scaled;
...
function calculateConfidence(query, productName) {
    ...
    if (pLower === qLower)
        return 0.95;
    if (pLower.includes(qLower) || qLower.includes(pLower))
        return 0.88;
    const qWords = qLower.split(/\s+/);
    const pWords = pLower.split(/[\s,]+/);
    let matching = 0;
    for (const qw of qWords) {
        if (pWords.some(pw => pw === qw))
            matching++;
    }
...
//# sourceMappingURL=openFoodFactsClient.js.map
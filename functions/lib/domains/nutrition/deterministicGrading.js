...
exports.calculateDeterministicGrades = calculateDeterministicGrades;
exports.gradeSingleFocus = gradeSingleFocus;
exports.getNutriScore = getNutriScore;
// ============================================================================
...
function calculatePositivePoints(nutrition) {
    ...
    const foodGroup = ((_a = nutrition.foodGroup) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || '';
    let fvnPoints = 0;
 ...
 */
function scoreComponent(value, thresholds, higherIsBetter = true) {
    if (higherIsBetter) {
...
function gradeBalanced(n) {
    ...
    const score = Math.round(proteinScore * 0.25 +
        fiberScore * 0.25 +
        satFatScore * 0.20 +
        sodiumScore * 0.15 +
        sugarScore * 0.15);
...
function generateBalancedInsight(score, n) {
    if (score >= 85)
        return 'Nutrient-dense with balanced macros and minimal negatives.';
    if (score >= 70)
        return 'Good nutritional profile with minor areas for improvement.';
    if (score >= 55)
        return 'Moderate nutrition - consider balancing with healthier options.';
    if (score >= 40)
        return 'Several nutritional concerns - best enjoyed occasionally.';
 ...
 */
function gradeMuscleBuilding(n) {
    var _a;
    const pros = [];
    const cons = [];
    ...
    const foodGroup = ((_a = n.foodGroup) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || '';
    const isCompleteProtein = foodGroup.includes('meat') ||
    ...
    const score = Math.round(proteinScore * 0.60 +
        qualityBonus +
        densityScore * 0.20 +
        carbScore * 0.10);
    if (n.protein >= 25)
        pros.push('Excellent protein for muscle synthesis');
    else if (n.protein >= 18)
        pros.push('Good protein content');
    if (isCompleteProtein)
        pros.push('Complete amino acid profile');
    if (proteinDensity > 10)
        pros.push('High protein density');
    if (n.protein < 8)
        cons.push('Very low protein');
    if (n.calories > 600 && n.protein < 20)
        cons.push('High calories without proportional protein');
    return {
        grade: scoreToGrade(score),
        score,
...
function gradeHeartHealth(n) {
    ...
    const score = Math.round(satFatScore * 0.30 +
        sodiumScore * 0.25 +
        transFatScore * 0.10 +
    ...
    if (n.saturatedFat <= 3)
        pros.push('Low saturated fat');
    if (n.sodium <= 400)
        pros.push('Low sodium');
    if (n.fiber >= 5)
        pros.push('Heart-healthy fiber');
    if ((n.potassium || 0) >= 300)
        pros.push('Good potassium source');
    if ((n.omega3 || 0) > 0.5)
        pros.push('Contains omega-3 fatty acids');
    if (n.saturatedFat > 7)
    ...
    if ((n.cholesterol || 0) > 150)
        cons.push('High cholesterol');
    return {
        grade: scoreToGrade(score),
        score,
        insight: score >= 85
 ...
 */
function gradeEnergyEndurance(n) {
    const pros = [];
    const cons = [];
    ...
    const score = Math.round(carbScore * 0.35 +
        ironScore * 0.20 +
        magScore * 0.15 +
        fiberScore * 0.20 +
        10 // base points
    ) - sugarPenalty;
    if (complexCarbRatio >= 0.7 && n.carbs >= 25)
        pros.push('Good complex carbohydrates');
    if ((n.iron || 0) >= 3)
        pros.push('Iron for oxygen transport');
    if ((n.magnesium || 0) >= 50)
        pros.push('Magnesium for ATP production');
    if (n.fiber >= 4)
        pros.push('Sustained energy release');
    if (n.sugar > 15 && n.fiber < 3)
        cons.push('High sugar may cause energy crash');
    if (n.carbs < 10)
        cons.push('Low carbs may limit endurance');
    return {
 ...
 */
function gradeWeightManagement(n) {
    const pros = [];
    const cons = [];
    ...
    const score = Math.round(densityScore * 0.30 +
        proteinScore * 0.30 +
        fiberScore * 0.25 +
        sugarScore * 0.15);
    if (calorieDensity <= 1.0)
        pros.push('Low calorie density');
    if (n.protein >= 18)
        pros.push('High protein for satiety');
    if (n.fiber >= 5)
        pros.push('Fiber promotes fullness');
    if (n.calories <= 200 && n.protein >= 10)
        pros.push('Low cal with good protein');
    if (calorieDensity > 2.0)
        cons.push('High calorie density');
    if (n.sugar > 15)
        cons.push('High sugar may increase hunger');
    if (n.calories > 500 && n.protein < 15)
        cons.push('High calories without satiety');
    return {
        grade: scoreToGrade(score),
        score,
        insight: score >= 85
...
function gradeBrainFocus(n) {
    ...
    const score = Math.round(omega3Score * 0.30 +
        glycemicScore * 0.25 +
        vitCScore * 0.20 +
        scoreComponent(n.fiber, { excellent: 6, good: 4, fair: 2, poor: 1 }) * 0.15 +
        10 + cholineBonus) - transFatPenalty;
    if ((n.omega3 || 0) >= 0.5)
        pros.push('Omega-3s for brain health');
    if (n.sugar < 8 && n.fiber >= 3)
        pros.push('Stable blood sugar for focus');
    if ((n.vitaminC || 0) >= 15)
        pros.push('Antioxidants protect brain cells');
    if (cholineBonus > 0)
        pros.push('Contains choline for neurotransmitters');
    if ((n.transFat || 0) > 0)
        cons.push('Trans fat harms brain function');
    if (n.sugar > 20)
        cons.push('High sugar impairs focus');
    return {
        grade: scoreToGrade(Math.max(0, score)),
        score: Math.max(0, score),
        insight: score >= 85
...
function gradeGutHealth(n) {
    ...
    const foodGroup = ((_a = n.foodGroup) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || '';
    const prebioticBonus = foodGroup.includes('legume') ||
    ...
    const fermentedBonus = foodGroup.includes('yogurt') ||
    ...
    const processedPenalty = ((n.transFat || 0) > 0 ? 10 : 0) +
        (n.sodium > 1000 ? 10 : 0);
    // Weight: fiber 70%, plus bonuses minus penalty
    const score = Math.round(fiberScore * 0.70 +
        20 + prebioticBonus + fermentedBonus) - processedPenalty;
    if (n.fiber >= 8)
        pros.push('Excellent fiber for microbiome');
    else if (n.fiber >= 5)
        pros.push('Good fiber content');
    if (prebioticBonus > 0)
        pros.push('Contains prebiotic fibers');
    if (fermentedBonus > 0)
        pros.push('Fermented - live probiotics');
    if (n.fiber < 2)
        cons.push('Very low fiber');
    if (processedPenalty > 10)
        cons.push('Processing may harm gut health');
    return {
        grade: scoreToGrade(Math.max(0, score)),
        score: Math.max(0, score),
        insight: score >= 85
...
function gradeBloodSugar(n) {
    ...
    const score = Math.round(sugarScore * 0.35 +
        fiberScore * 0.25 +
        proteinScore * 0.20 +
        carbQualityScore * 0.20) - carbLoadPenalty;
    if (n.sugar < 5)
        pros.push('Very low sugar');
    if (n.fiber >= 5)
        pros.push('Fiber slows glucose absorption');
    if (n.protein >= 15 && n.carbs < 30)
        pros.push('Protein moderates blood sugar');
    if (complexCarbRatio >= 0.7)
        pros.push('Complex carbohydrates');
    if (n.sugar > 15)
        cons.push('High sugar content');
    if (n.carbs > 50 && n.fiber < 5)
        cons.push('High carb load without fiber');
    if (complexCarbRatio < 0.5)
        cons.push('Mostly simple carbs');
    return {
        grade: scoreToGrade(Math.max(0, score)),
        score: Math.max(0, score),
        insight: score >= 85
...
function gradeBoneJoint(n) {
    ...
    const score = Math.round(calciumScore * 0.35 +
        vitDScore * 0.25 +
        magScore * 0.20 +
        15 + omega3Bonus) - Math.round(sodiumPenalty);
    if ((n.calcium || 0) >= 150)
        pros.push('Good calcium for bones');
    if ((n.vitaminD || 0) >= 2)
        pros.push('Vitamin D aids calcium absorption');
    if ((n.magnesium || 0) >= 50)
        pros.push('Magnesium for bone matrix');
    if ((n.omega3 || 0) >= 0.5)
        pros.push('Omega-3 supports joint health');
    if ((n.calcium || 0) < 50)
        cons.push('Low calcium content');
    if (n.sodium > 1000)
        cons.push('High sodium may deplete calcium');
    return {
        grade: scoreToGrade(Math.max(0, score)),
        score: Math.max(0, score),
        insight: score >= 85
 ...
 */
function gradeAntiInflammatory(n) {
    var _a;
    const pros = [];
    const cons = [];
    ...
    const foodGroup = ((_a = n.foodGroup) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || '';
    const plantBonus = foodGroup.includes('fruit') ||
        foodGroup.includes('vegetable') ||
        foodGroup.includes('berry') ? 10 : 0;
    // Weight: omega3 25%, satFat 25%, sugar 20%, antioxidants 15%, fiber 15%
    const score = Math.round(omega3Score * 0.25 +
        satFatScore * 0.25 +
        sugarScore * 0.20 +
        antioxidantScore * 0.15 +
        fiberScore * 0.15 +
        plantBonus) - transFatPenalty;
    if ((n.omega3 || 0) >= 0.5)
        pros.push('Anti-inflammatory omega-3s');
    if (n.saturatedFat <= 3)
        pros.push('Low saturated fat');
    if ((n.vitaminC || 0) >= 15)
        pros.push('Antioxidants reduce inflammation');
    if (plantBonus > 0)
        pros.push('Plant compounds with anti-inflammatory effects');
    if ((n.transFat || 0) > 0)
        cons.push('Trans fat promotes inflammation');
    if (n.saturatedFat > 7)
        cons.push('High saturated fat is pro-inflammatory');
    if (n.sugar > 15)
        cons.push('Excess sugar increases inflammation');
    return {
        grade: scoreToGrade(Math.max(0, score)),
        score: Math.max(0, score),
        insight: score >= 85
 ...
 */
function calculateSatietyScore(n) {
    const servingGrams = n.servingGrams || 100;
 ...
 */
function calculateInflammatoryIndex(n) {
 ...
 */
function calculateDeterministicGrades(nutrition) {
 ...
 */
function gradeSingleFocus(nutrition, focus) {
    switch (focus) {

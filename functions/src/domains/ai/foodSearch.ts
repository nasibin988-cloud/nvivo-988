...
export interface FoodSearchResult {
  ...
  nutrition: Partial<CompleteNutrition> & EssentialNutrition;
}
 ...
 */
function getNutrientIdsForLevel(level: NutritionDetailLevel): number[] {
  const essential = [
...
function extractNutrition(
...
): Partial<CompleteNutrition> & EssentialNutrition {
  // Essential (always included)
  const essential: EssentialNutrition = {
  ...
  const extended: ExtendedNutrition = {
    ...essential,
  ...
  const complete: CompleteNutrition = {
    ...extended,
    // Fat details
 ...
 */
function getFallbackResults(query: string, detailLevel: NutritionDetailLevel = 'essential'): FoodSearchResult[] {
  // Comprehensive fallback data with extended nutrition
  const commonFoods: Array<{
    name: string;
    essential: EssentialNutrition;
    extended?: Partial<ExtendedNutrition>;
    complete?: Partial<CompleteNutrition>;
  }> = [
    {
  ...
  return filtered.map((food) => {
    let nutrition: Partial<CompleteNutrition> & EssentialNutrition = { ...food.essential };
...
// Export the secret for use in Cloud Function
export { usdaApiKey };

...
import type { CompleteNutrition, USDAFoodItem, USDANutrient } from '../types';
...
const NUTRIENT_ID_MAP: Record<number, keyof CompleteNutrition> = {
 ...
 */
export async function searchUSDA(query: string): Promise<USDASearchResult | null> {
  const apiKey = getApiKey();

  try {
    const response = await fetch(
      `${USDA_BASE_URL}/foods/search?query=${encodeURIComponent(query)}&api_key=${apiKey}&pageSize=5&dataType=Survey%20(FNDDS),Foundation,SR%20Legacy`
    );
 ...
 */
export async function getUSDAByFdcId(fdcId: number): Promise<USDASearchResult | null> {
  const apiKey = getApiKey();
  ...
  try {
    ...
    const food = await response.json() as USDAFoodItem;
 ...
 */
export async function batchSearchUSDA(queries: string[]): Promise<Map<string, USDASearchResult>> {
  ...
  const map = new Map<string, USDASearchResult>();
 ...
 */
function findBestMatch(query: string, foods: USDAFoodItem[]): USDAFoodItem | null {
 ...
 */
function mapUSDAToNutrition(nutrients: USDANutrient[]): CompleteNutrition {
  ...
  for (const nutrient of nutrients) {
    ...
    if (key && key in nutrition) {
      (nutrition[key] as number) = roundNutrient(nutrient.value, key);
    }
  }
 ...
 */
function scaleToServing(nutrition: CompleteNutrition, servingGrams: number): CompleteNutrition {
  if (servingGrams === 100) return nutrition;

  const scale = servingGrams / 100;
  const scaled: CompleteNutrition = { ...nutrition };

  for (const key of Object.keys(scaled) as (keyof CompleteNutrition)[]) {
 ...
 */
function calculateConfidence(query: string, description: string): number {
  const queryLower = query.toLowerCase();
  const descLower = description.toLowerCase();

  // Exact match
  if (descLower === queryLower) return 0.98;

  // Contains full query
  if (descLower.includes(queryLower)) return 0.92;

  // Query contains description
  if (queryLower.includes(descLower)) return 0.88;
  ...
  for (const qw of queryWords) {
    if (descWords.some(dw => dw === qw)) matching++;
  }
...
// Export the secret for Cloud Function access
export { usdaApiKey };

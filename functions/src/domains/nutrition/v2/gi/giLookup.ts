...
import type { GIResult, GIBand, GLBand, CompleteNutrition } from '../types';
import { GI_DATABASE, CATEGORY_DEFAULTS, type GIEntry, type GICategory } from './giDatabase';
...
export function lookupGI(
  ...
  // 1. Try exact match
  let entry: GIEntry | undefined = GI_DATABASE[normalizedName];
  let source: GIResult['source'] = 'exact';
  ...
  if (!entry) {
    ...
    const defaultGI = CATEGORY_DEFAULTS[category];
    entry = {
      gi: defaultGI,
      servingGrams: 100,
...
export function batchLookupGI(
  ...
  const results = new Map<string, GIResult>();
 ...
 */
export function getGIBand(gi: number): GIBand {
  if (gi <= GI_LOW_MAX) return 'low';
  if (gi <= GI_MEDIUM_MAX) return 'medium';
  return 'high';
}
 ...
 */
export function getGLBand(gl: number): GLBand {
 ...
 */
function calculateNetCarbs(nutrition: CompleteNutrition, servingGrams: number): number {
 ...
 * Find entry by alias
 */
function findByAlias(normalizedName: string): GIEntry | null {
  for (const [_, entry] of Object.entries(GI_DATABASE)) {
 ...
 */
function findFuzzyMatch(normalizedName: string): { entry: GIEntry; score: number } | null {
  const queryWords = normalizedName.split(/\s+/);
  let bestMatch: { entry: GIEntry; score: number } | null = null;

  for (const [name, entry] of Object.entries(GI_DATABASE)) {
 ...
 */
function inferCategory(name: string, nutrition: CompleteNutrition): GICategory {
  const nameLower = name.toLowerCase();

  // Check for category keywords
  const categoryKeywords: Record<GICategory, string[]> = {
 ...
 */
function calculateConfidence(entry: GIEntry, source: GIResult['source']): number {
  const baseConfidence: Record<GIEntry['confidence'], number> = {
    high: 0.95,
    medium: 0.80,
    low: 0.65,
  };

  const sourceMultiplier: Record<GIResult['source'], number> = {
    exact: 1.0,
    fuzzy: 0.85,
    category: 0.60,
    default: 0.40,
 ...
 */
export function hasRelevantGI(nutrition: CompleteNutrition): boolean {
 ...
 */
export function getGIExplanation(giResult: GIResult): string {
  const giBandText: Record<GIBand, string> = {
  ...
  const glBandText: Record<GLBand, string> = {
 ...
 */
export function calculateMealGI(
  items: Array<{ gi: number; carbsInServing: number }>
): { gi: number; giBand: GIBand } {
  ...
  const weightedGI = items.reduce(
    (sum, item) => sum + (item.gi * item.carbsInServing),
    0
  ) / totalCarbs;
 ...
 */
export function calculateMealGL(
  items: Array<{ gl: number }>
): { gl: number; glBand: GLBand } {

...
import type { FoodIntelligence, FoodFocusGrades, FocusGrade } from '../ai/foodAnalysis';

// ============================================================================
// TYPES
// ============================================================================

// Generic entry type - we'll extract fields dynamically
type AnyFoodEntry = Record<string, unknown>;
...
const FOCUS_KEY_MAP: Record<string, keyof FoodFocusGrades> = {
  // Direct mappings
  balanced: 'balanced',
...
const PROCESSING_TO_NOVA: Record<string, 1 | 2 | 3 | 4> = {
  unprocessed: 1,
 ...
 */
function convertDietaryFlagsToTags(flags: Record<string, boolean>): string[] {
  const tags: string[] = [];
  if (flags.isVegetarian) tags.push('vegetarian');
  if (flags.isVegan) tags.push('vegan');
  if (flags.isGlutenFree) tags.push('gluten_free');
  if (flags.isDairyFree) tags.push('dairy_free');
  if (flags.isKeto) tags.push('keto');
  if (flags.isPaleo) tags.push('paleo');
  if (flags.isWholeFoodPlantBased) tags.push('whole_food_plant_based');
  if (flags.isLowCarb) tags.push('low_carb');
  if (flags.isHighProtein) tags.push('high_protein');
  if (flags.isLowSodium) tags.push('low_sodium');
  return tags;
}
 ...
 */
function convertFocusGrades(
  rawGrades: Record<string, unknown>
): FoodFocusGrades | undefined {
  const result: FoodFocusGrades = {};
 ...
 */
function extractIntelligence(entry: AnyFoodEntry): FoodIntelligence | null {
  const intelligence: FoodIntelligence = {};

  // Extract insight from various locations
  const insight = getNestedValue(
    entry,
    'insight',
    'healthMetrics.insight',
    'focusGrades.general.reasoning',
    'focusGrades.balanced.insight'
  ) as string | undefined;
  if (insight) {
  ...
  const focusGrades = getNestedValue(entry, 'focusGrades') as Record<string, unknown> | undefined;
  if (focusGrades) {
    intelligence.focusGrades = convertFocusGrades(focusGrades);
  }

  // Extract glycemic data from multiple possible locations
  const gi = getNestedValue(
    entry,
    'glycemicIndex',
    'healthMetrics.glycemicIndex',
    'derivedScores.glycemicIndex'
  ) as number | undefined;
  if (typeof gi === 'number') {
  ...
  const gl = getNestedValue(
    entry,
    'glycemicLoad',
    'healthMetrics.glycemicLoad',
    'derivedScores.glycemicLoad'
  ) as number | undefined;
  if (typeof gl === 'number') {
  ...
  const satiety = getNestedValue(
    entry,
    'satietyScore',
    'healthMetrics.satietyScore',
    'derivedScores.satietyScore'
  ) as number | undefined;
  ...
  const inflammatory = getNestedValue(
    entry,
    'inflammatoryIndex',
    'inflammatoryScore',
    'healthMetrics.inflammatoryScore',
    'healthMetrics.inflammatoryIndex',
    'derivedScores.inflammatoryIndex'
  ) as number | undefined;
  ...
  let nova = getNestedValue(
    entry,
    'novaClass',
    'healthMetrics.novaClass',
    'derivedScores.novaClass'
  ) as number | undefined;

  if (!nova) {
    const processingLevel = getNestedValue(
      entry,
      'processingLevel',
      'healthMetrics.processingLevel'
    ) as string | undefined;
    if (processingLevel) {
  ...
  if (nova && [1, 2, 3, 4].includes(nova)) {
    intelligence.novaClass = nova as 1 | 2 | 3 | 4;
  }

  // Extract dietary tags from multiple sources
  let tags = getNestedValue(entry, 'dietaryTags', 'tags') as string[] | undefined;

  if (!tags || tags.length === 0) {
    const dietaryFlags = getNestedValue(
      entry,
      'dietaryFlags',
      'healthMetrics.dietaryFlags'
  ...
  // Extract allergens
  const allergens = getNestedValue(
    entry,
    'allergens',
    'healthMetrics.allergens'
  ) as string[] | undefined;
  ...
  const nutrientDensity = getNestedValue(
    entry,
    'nutrientDensityScore',
    'nutrientDensity',
    'healthMetrics.nutrientDensity',
    'derivedScores.nutrientDensityScore'
  ) as number | undefined;
  ...
  // Extract ORAC score
  const orac = getNestedValue(
    entry,
    'oracScore',
    'healthMetrics.oracScore',
    'derivedScores.oracScore'
  ) as number | undefined;
  ...
  const foodGroup = getNestedValue(
    entry,
    'foodGroup',
    'foodGroups',
    'category'
  ...
  // Extract omega ratio
  const omegaRatio = getNestedValue(entry, 'omegaRatio') as number | undefined;
 ...
 */
function extractName(entry: AnyFoodEntry): string | undefined {
...
const FOOD_INTELLIGENCE: Map<string, FoodIntelligence> = new Map();

/**
 * Add entries from a data source to the lookup
 */
function loadEntries(data: unknown): number {
  if (!data || typeof data !== 'object') return 0;

  const entries = data as Record<string, AnyFoodEntry>;
  let count = 0;
 ...
 */
function loadAllEnhancedData(): void {
 ...
 */
export function getFoodIntelligence(foodName: string): FoodIntelligence | null {
 ...
 */
export function getFocusGradeForFood(
  foodName: string,
  focusId: keyof FoodFocusGrades
): FocusGrade | null {
 ...
 */
export function hasIntelligence(foodName: string): boolean {
 ...
 */
export function getIntelligenceFoodCount(): number {

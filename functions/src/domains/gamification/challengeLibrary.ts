 ...
 */
export function getChallengesByTag(tag: string): ChallengeTemplate[] {
  return CHALLENGE_LIBRARY.filter((c) => c.tags.includes(tag));
}
 ...
 */
export function templateToMicroWin(template: ChallengeTemplate): {
  id: string;
  title: string;
  description: string;
  completed: boolean;
 ...
 */
export function generateDailyMicroWins(
  count: number = 3,
  options?: Parameters<typeof selectRandomChallenges>[1]
): ReturnType<typeof templateToMicroWin>[] {

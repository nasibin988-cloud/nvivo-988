...
export interface SeedCognitiveHealthOptions {
  patientId: string;
}

export async function seedCognitiveHealth(options: SeedCognitiveHealthOptions): Promise<void> {
...
export async function clearCognitiveHealth(patientId: string): Promise<void> {
  const db = getFirestore();
